...
export function createPartyCard(title, party) {
  if (!party) {
    return `
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-6">
        ...
        <p class="text-gray-500 dark:text-gray-400">No disponible</p>
      </div>
    `
  }
  ...
  return `
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-6">
      ...
      <p class="text-lg font-semibold text-gray-800 dark:text-gray-100">${escapeHtml(party.name || 'Sin nombre')}</p>
      <p class="text-gray-600 dark:text-gray-300 flex items-center gap-2">
        <span>${escapeHtml(taxId || 'Sin NIF')}</span>
        ${taxId ? `
          <button
