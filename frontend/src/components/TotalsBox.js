...
export function createTotalsBox(taxes, totals) {
  ...
  const taxRows = taxes && taxes.length > 0
    ? taxes.map(tax => `
        <div class="flex justify-between text-sm">
          <span class="text-gray-600 dark:text-gray-400">IVA ${escapeHtml(tax.rate)}% (Base: ${escapeHtml(formatCurrency(tax.base))})</span>
  ...
  return `
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-6">
      <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Totales</h3>
      ...
      <div class="space-y-2">
        ...
        ${totals.taxesWithheld > 0 ? `
          <div class="flex justify-between text-sm">
            <span class="text-gray-600 dark:text-gray-400">Retenciones</span>
